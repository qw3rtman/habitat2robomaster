...
from train_pointgoal_dagger_student_rnn import get_run_name
...
if __name__ == '__main__':
    ...
    dagger_dir = root/'data'/'dagger'/run_name
    ...
    while True:
        # 1. run train script
        print('training...')
        subprocess.run(['python', 'train_pointgoal_dagger_student_rnn.py']+sys.argv[1:]+['--dagger_dir', dagger_dir])
        ...
        print('dagger...')
        subprocess.run(['python', 'dagger_collector.py', '--model', checkpoint/'model_latest.t7', '--epoch', str(run.summary['epoch'])])
